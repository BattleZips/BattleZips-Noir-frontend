...
/**
 * Load in a common reference string
 * 
 * @param {number} circSize - size of circuit 
 * @returns CRS
 */
...
/**
 * Setup a Barrentenberg prover by passing in a serialized 
 * 
 * @param {any} serialized_circuit - serialized Noir ciruit
 * @param {number} circSize - size of the circuit 
 * @returns - Barretenberg prover
 */
export default async function setup_generic_prover(serialized_circuit: any, circSize: number = 8092) {
  ...
  await standardExampleProver.initCircuitDefinition(serialized_circuit);
